...
async function getEmployeeLeaves() {
  ...
  response.data.forEach((leave) => {
    const leaveType = leave.leave_type_id.leave_name;
    if (!leavesByType[leaveType]) {
      leavesByType[leaveType] = 0;
    }
  ...
  if (response.status === 200) {
    ...
    if (Array.isArray(response.data)) {
      response.data.forEach((balance) => {
        const leaveName = balance.leave_type?.leave_name || "Unknown";
        const allocated = balance.allocated || balance.allocated_leaves || 0;
        const used = balance.used || balance.used_leaves || 0;
        const available = balance.available || allocated - used || 0;

        console.log(
          `- ${leaveName}: ${available} available (${allocated} allocated, ${used} used)`
        );
      });
    } else {
      console.log("Unexpected format for leave balances:", response.data);
    }
  } else {
    console.log("Failed to get leave balances:", response.data);
  }
}
