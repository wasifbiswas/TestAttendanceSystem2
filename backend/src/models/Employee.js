...
employeeSchema.pre(['deleteOne', 'findOneAndDelete'], async function(next) {
  try {
    ...
    const deletedAttendance = await Attendance.deleteMany({ emp_id: employeeId });
    console.log(`Deleted ${deletedAttendance.deletedCount} attendance records for employee ${employeeId}`);
    
    // Clean up leave requests
    const deletedLeaveRequests = await LeaveRequest.deleteMany({ emp_id: employeeId });
    console.log(`Deleted ${deletedLeaveRequests.deletedCount} leave requests for employee ${employeeId}`);
    
    // Clean up leave balances
    const deletedLeaveBalances = await LeaveBalance.deleteMany({ emp_id: employeeId });
    console.log(`Deleted ${deletedLeaveBalances.deletedCount} leave balances for employee ${employeeId}`);
    ...
    console.log(`Updated ${updatedSubordinates.modifiedCount} subordinate employees`);
