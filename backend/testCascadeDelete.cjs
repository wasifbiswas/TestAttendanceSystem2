...
async function testCascadeDelete() {
  try {
    ...
    if (!department || !employeeRole || !leaveType) {
      console.log(
        "❌ Required data not found (department, employee role, or leave type)"
      );
      return;
    }
    ...
    console.log(
      `   ✅ Created employee: ${employee._id} (${employee.employee_code})`
    );
    ...
    if (allCleaned) {
      ...
      if (countsAfter.users > 0)
        console.log(`     - ${countsAfter.users} user(s) still exist`);
      if (countsAfter.userRoles > 0)
        console.log(`     - ${countsAfter.userRoles} user role(s) still exist`);
      if (countsAfter.employees > 0)
        console.log(`     - ${countsAfter.employees} employee(s) still exist`);
      if (countsAfter.attendance > 0)
        console.log(
          `     - ${countsAfter.attendance} attendance record(s) still exist`
        );
      if (countsAfter.leaveBalances > 0)
        console.log(
          `     - ${countsAfter.leaveBalances} leave balance(s) still exist`
        );
      if (countsAfter.leaveRequests > 0)
        console.log(
          `     - ${countsAfter.leaveRequests} leave request(s) still exist`
        );
    }
    ...
    console.log(
      "   ✅ Future user deletions will automatically clean up all related data"
    );
