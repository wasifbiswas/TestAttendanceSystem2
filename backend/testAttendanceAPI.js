...
const req1 = http.request(options1, (res) => {
  ...
  res.on("end", () => {
    try {
      ...
      console.log(
        `   Total Employees: ${jsonData.employees} (should include ALL employees including managers)`
      );
      console.log(`   Total Users: ${jsonData.users}`);
      console.log(`   Total Departments: ${jsonData.departments}`);
      console.log(
        `   Attendance: Present=${jsonData.attendance.present}, Absent=${jsonData.attendance.absent}, On Leave=${jsonData.attendance.onLeave}`
      );
  ...
  const req2 = http.request(options2, (res) => {
    ...
    res.on("end", () => {
      try {
        ...
        console.log(
          `   Employees: ${jsonData.employees.count} (should exclude managers)`
        );
        ...
        // Verify the logic
        const totalByRole =
          jsonData.employees.count +
          jsonData.managers.count +
          jsonData.admins.count;
        console.log(
        ...
        console.log("\n🎯 EXPECTED BEHAVIOR:");
        console.log(
          "✅ Total Employees section should show ALL employees (including managers)"
        );
        console.log("✅ Employee Summary by Role section should show:");
        console.log(
          "   - Employees: only regular employees (excluding managers)"
        );
