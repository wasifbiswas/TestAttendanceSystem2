  ...
  try {
    ...
    for (const userRole of userRoles) {
      ...
      if (!userExists || !roleExists) {
        ...
        cleanedUserRoles++;
        console.log(
          `   ❌ Removed UserRole ${userRole._id} (user: ${
            userExists ? "✓" : "✗"
          }, role: ${roleExists ? "✓" : "✗"})`
        );
      }
    ...
    console.log(
      `   ✅ Cleaned ${cleanedUserRoles} orphaned user role assignments`
    );
    ...
    for (const employee of employees) {
      ...
      if (!userExists) {
        ...
        cleanedEmployees++;
        console.log(
          `   ❌ Removed Employee ${employee._id} (${employee.employee_code}) - user deleted`
        );
      } else if (!deptExists) {
        console.log(
          `   ⚠️  Employee ${employee._id} (${employee.employee_code}) references deleted department ${employee.dept_id}`
        );
    ...
    for (const attendance of attendanceRecords) {
      ...
      if (!employeeExists) {
        ...
        cleanedAttendance++;
        console.log(
          `   ❌ Removed Attendance ${attendance._id} - employee ${attendance.emp_id} deleted`
        );
      }
    }
    console.log(
      `   ✅ Cleaned ${cleanedAttendance} orphaned attendance records`
    );
    ...
    for (const leaveRequest of leaveRequests) {
      ...
      if (!employeeExists) {
        ...
        console.log(
          `   ❌ Removed Leave Request ${leaveRequest._id} - employee ${leaveRequest.emp_id} deleted`
        );
      }
    }
    console.log(
      `   ✅ Cleaned ${cleanedLeaveRequests} orphaned leave requests`
    );
    ...
    for (const leaveBalance of leaveBalances) {
      ...
      if (!employeeExists) {
        ...
        console.log(
          `   ❌ Removed Leave Balance ${leaveBalance._id} - employee ${leaveBalance.emp_id} deleted`
        );
      }
    }
    console.log(
      `   ✅ Cleaned ${cleanedLeaveBalances} orphaned leave balances`
    );
    ...
    for (const notification of notifications) {
      ...
      for (const recipient of notification.recipients) {
        ...
        if (userExists) {
          ...
          hasChanges = true;
          console.log(
            `   ❌ Removed recipient ${recipient.user_id} from notification ${notification._id}`
          );
        }
      }
      ...
      if (hasChanges) {
        if (validRecipients.length === 0) {
          ...
          console.log(
            `   🗑️  Deleted notification ${notification._id} - no valid recipients left`
          );
        } else {
          ...
          console.log(
            `   🔄 Updated notification ${notification._id} - removed ${
              originalRecipientCount - validRecipients.length
            } orphaned recipients`
          );
        }
    ...
    console.log(
      `   ✅ Cleaned ${cleanedNotifications} notifications, removed ${totalRecipientsRemoved} orphaned recipients`
    );
    ...
    console.log("\n💡 Future Prevention:");
    console.log(
      "   ✅ Cascade delete hooks implemented in User and Employee models"
    );
    console.log(
