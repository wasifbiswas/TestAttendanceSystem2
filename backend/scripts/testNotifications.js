...
async function verifyNotificationCreated(notificationId, uniqueId) {
  try {
    console.log(
      `Verifying notification creation (ID: ${notificationId}, Unique ID: ${uniqueId})...`
    );
    ...
    if (data.success && data.data && data.data.length > 0) {
      ...
      const foundByUniqueId = data.data.find((n) =>
        n.title.includes(`[${uniqueId}]`)
      );
      ...
      if (foundById) {
        ...
        console.error(
          "❌ Notification was created but not found in user notifications"
        );
        console.log(
          "Available notifications:",
          data.data.map((n) => ({ id: n._id, title: n.title }))
        );
        return false;
      }
    } else {
...
async function runTests() {
  ...
  if (!results.apiConnectivity) {
    console.error(
      "❌ API connectivity test failed. Continuing with other tests..."
    );
  }
  ...
  console.log(
    `API Connectivity:        ${
      results.apiConnectivity ? "✅ PASSED" : "❌ FAILED"
    }`
  );
  console.log(
    `Login Authentication:    ${results.login ? "✅ PASSED" : "❌ FAILED"}`
  );
  console.log(
    `Notifications Endpoint:  ${
      results.notificationsEndpoint ? "✅ PASSED" : "❌ FAILED"
    }`
  );
  console.log(
    `Create Notification:     ${
      results.createNotification ? "✅ PASSED" : "❌ FAILED"
    }`
  );
  console.log("===============================");
