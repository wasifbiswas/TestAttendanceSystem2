...
async function validateDataIntegrity() {
  try {
      ...
      for (const recipient of notification.recipients) {
        ...
        if (!userExists) {
          ...
          issues.push({
            ...
            data: { notificationId: notification._id, recipientUserId: recipient.user_id }
          });
        }
      }
    ...
    console.log(`   Found ${notificationsWithOrphanedRecipients} notifications with orphaned recipients`);
    ...
    if (issues.length === 0) {
      ...
      console.log(`   - Notifications with orphaned recipients: ${notificationsWithOrphanedRecipients}`);
